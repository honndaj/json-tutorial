...
static void test_parse_array() {
    ...
    EXPECT_EQ_STRING("abc", lept_get_string(lept_get_array_element(&v, 4)), strlen("abc"));
    lept_free(&v);
    ...
    for (i = 0; i < 4; i++) {
        lept_value *t = lept_get_array_element(&v, i);
        EXPECT_EQ_INT(LEPT_ARRAY, lept_get_type(t));
        EXPECT_EQ_SIZE_T(i, lept_get_array_size(t));
        for (j = 0; j < lept_get_array_size(t); j++) {
            EXPECT_EQ_DOUBLE(j, lept_get_number(lept_get_array_element(t, j)));
        }
    }
    lept_free(&v);
}
