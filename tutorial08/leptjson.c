        ...
        case LEPT_OBJECT:
            ...
            for (i = 0;i <lhs->u.o.size; i++) {
                ...
                if (index == LEPT_KEY_NOT_EXIST)
                    return 0;
                value = lept_find_object_value(rhs, lhs->u.o.m[i].k, lhs->u.o.m[i].klen);
                if (lept_is_equal(value, &lhs->u.o.m[i].v) == 0)
                    return 0;
            }
            return 1;
        default:
...
lept_value* lept_insert_array_element(lept_value* v, size_t index) {
    ...
    if (index < lept_get_array_size(v)) {
        size_t i;
        if (lept_get_array_size(v) == lept_get_array_capacity) {
            lept_reserve_array(v, lept_get_array_capacity(v) + 1);
        }
        for (i = lept_get_array_size(v); i > index; i--) {
...
lept_value* lept_set_object_value(lept_value* v, const char* key, size_t klen) {
    size_t find;
    assert(v != NULL && v->type == LEPT_OBJECT && key != NULL);
    find = lept_find_object_index(v, key, klen);
    if (find != LEPT_KEY_NOT_EXIST) return &v->u.o.m[find].v;
